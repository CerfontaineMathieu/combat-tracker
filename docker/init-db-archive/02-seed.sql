...
CREATE OR REPLACE FUNCTION parse_actions(actions_text TEXT)
RETURNS JSONB AS $$
DECLARE
    result JSONB := '[]'::jsonb;
    action_lines TEXT[];
...
BEGIN
    ...
    LOOP
        ...
        -- Find the first period followed by a space (action name delimiter)
...
CREATE OR REPLACE FUNCTION parse_legendary_actions(actions_text TEXT)
RETURNS JSONB AS $$
DECLARE
    result JSONB := '[]'::jsonb;
    action_lines TEXT[];
...
BEGIN
    ...
    LOOP
        ...
        -- Check for cost in parentheses like "(coûte 2 actions)"
        cost := 1;
...
CREATE OR REPLACE FUNCTION parse_traits(description_text TEXT)
RETURNS JSONB AS $$
DECLARE
    result JSONB;
...
BEGIN
    ...
    -- Parse remaining text as special abilities
...
SELECT
    ...
    parse_legendary_actions(legendary_actions),
    parse_traits(description)
FROM monsters_import;

-- Cleanup
DROP TABLE monsters_import;
DROP FUNCTION parse_actions;
DROP FUNCTION parse_legendary_actions;
DROP FUNCTION parse_traits;
...
BEGIN
    ...
    RAISE NOTICE 'Successfully imported % monsters with structured JSONB data', monster_count;
END $$;